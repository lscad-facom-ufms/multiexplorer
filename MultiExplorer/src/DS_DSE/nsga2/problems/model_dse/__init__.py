...
class DS_DSE(Problem):
    ...
    def __init__(self, DSEDefinitions, projectFolder, path_db = DbSelector(inputName=sys.argv[1]).select_db()):
        ...
        print "\nOriginal Power Density : ",(float(self.dict_entry["parameters"]["power_orig"][0])/float(self.dict_entry["parameters"]["area_orig"][0]))
    ...
    def __dominates(self, individual2, individual1):
        ...
        # se os dois individuos são viaveis
            ...
            worse_than_other = self.dse_definitions.powerDensity(individual1) <= self.dse_definitions.powerDensity(individual2) and pf2 <= pf1
            better_than_other = self.dse_definitions.powerDensity(individual1) < self.dse_definitions.powerDensity(individual2) or pf2 < pf1
